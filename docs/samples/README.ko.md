# 브랜드 샘플

[English](README.md) · [한국어](README.ko.md) · [문서](../README.ko.md) · [Logopia](../../README.ko.md)

[**현재 쇼케이스 보드 크게 보기 →**](../../assets/logopia-showcase.png) · [현재 갤러리](../gallery.ko.md#showcase) · [전체 PNG 목록](../showcase/2026-09/README.ko.md)

## 2026년 9월 컬렉션

가상 브랜드 10개를 앱 아이콘 예제 6개와 함께 새롭게 표현했습니다. 개성 있는 레터링, 간결한 마크와 한글 이름을 살펴보세요. 아래 링크를 누르면 현재 PNG가 열립니다.

| 브랜드 | 현재 이미지 |
|---|---|
| LUMA | [01-luma.png](../showcase/2026-09/images/01-luma.png) |
| LOOP LAB | [02-loop-lab.png](../showcase/2026-09/images/02-loop-lab.png) |
| 고요 | [03-goyo.png](../showcase/2026-09/images/03-goyo.png) |
| BREAD & BLOOM | [04-bread-bloom.png](../showcase/2026-09/images/04-bread-bloom.png) |
| KITE | [05-kite.png](../showcase/2026-09/images/05-kite.png) |
| MISO | [06-miso.png](../showcase/2026-09/images/06-miso.png) |
| NORTHLINE / NL | [07-northline.png](../showcase/2026-09/images/07-northline.png) |
| 물결 | [08-mulgyeol.png](../showcase/2026-09/images/08-mulgyeol.png) |
| FERN | [09-fern.png](../showcase/2026-09/images/09-fern.png) |
| NOVA NOTES | [10-nova-notes.png](../showcase/2026-09/images/10-nova-notes.png) |

래스터 예제 이미지입니다. 이 컬렉션의 파일과 제작 출처는 [현재 목록](../showcase/2026-09/README.ko.md)과 [매니페스트](../showcase/2026-09/manifest.json)에서 확인하실 수 있습니다. 이전 요청서·검수·전달 패키지는 아래 보관 이미지에 속합니다.

## 레터링을 요청해 보세요

> $logo-land LUMA의 경쾌한 다색 워드마크를 만들어 주세요. 정확한 글자는 LUMA이며, 굵은 글자 모양과 간격으로 개성을 표현해 주세요. 별도 아이콘, 슬로건이나 추가 글자는 넣지 말아 주세요.

> $logo-land 고요의 한글 워드마크를 만들어 주세요. 정확한 글자는 고요이며, 둥근 획과 넉넉한 글자 간격으로 차분한 분위기를 표현해 주세요. 별도 심볼이나 추가 문구는 넣지 말아 주세요.

[레터링 안내](../../skills/logo-land/references/typography.md) · [로고 방향](../../skills/logo-land/references/logo-directions.md)

<a id="historical-samples"></a>

## 이전 컬렉션 — 보관 자료

앞선 컬렉션의 예제 10개를 보존했습니다. 개별 설명에는 당시 원본 이미지, 요청과 제공 파일이 남아 있으며, 새 쇼케이스 PNG의 제작 기록과는 다릅니다.

| 이전 샘플 | 당시 유형 |
|---|---|
| [LUMA](items/01-luma/README.ko.md) | 워드마크 |
| [LOOP LAB](items/02-loop-lab/README.ko.md) | 모노그램 |
| [고요](items/03-goyo/README.ko.md) | 한글 조합형 |
| [BREAD & BLOOM](items/04-bread-bloom/README.ko.md) | 엠블럼 |
| [KITE](items/05-kite/README.ko.md) | 추상형 |
| [MISO](items/06-miso/README.ko.md) | 마스코트 |
| [NORTHLINE · NL](items/07-northline/README.ko.md) | 레터마크 |
| [물결](items/08-mulgyeol/README.ko.md) | 한글 워드마크 |
| [FERN](items/09-fern/README.ko.md) | 심볼 |
| [NOVA NOTES](items/10-nova-notes/README.ko.md) | 조합형 |

이전 브랜드 원본은 불투명 배경입니다. 투명 배경 전달은 별도의 [GROVE 투명 예제](transparency.ko.md)를 확인해 주세요.

[이전 비주얼 갤러리](../gallery.ko.md#brands) · [이전 인터랙티브 갤러리 — 로컬에서 열기](index.html) · [색상·타이포그래피 사례](../colors/README.ko.md)
